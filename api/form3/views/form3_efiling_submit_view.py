...
class Form3EFilingSubmitView(generics.GenericAPIView):
    ...
    def get_notice_of_cross_appeal_for_user(self, notice_of_cross_appeal_id, uid):
        try:
            notice_of_cross_appeal_query = NoticeOfCrossAppeal.objects.get(id=notice_of_cross_appeal_id, user_id=uid)
            return notice_of_cross_appeal_query
        except (NoticeOfCrossAppeal.DoesNotExist):
            logger.debug(no_record_found)
            return


    def _get_pdf_content(self, notice_of_cross_appeal, document_type):
        outgoing_documents = [] 
        #Modify If more than one form type exist in the future       
        
        current_document_type = "NCA"
        try:                      
            prepared_pdf = FormPdf.objects.get(
                notice_of_cross_appeal_id=notice_of_cross_appeal.id, pdf_type=f"{current_document_type}"
            )
        ...
        document_json.update({"applicationId": notice_of_cross_appeal.id})
    ...
    def put(self, request, notice_of_cross_appeal_id):
        uid = request.user.id
        body = request.data
        ...
        if not notice_of_cross_appeal.submission_id or  not notice_of_cross_appeal.transaction_id:
            return HttpResponseNotFound("no record found")

        notice_of_cross_appeal.package_number = body.get("packageNumber")
        notice_of_cross_appeal.package_url = body.get("packageUrl")
        notice_of_cross_appeal.status="Submitted"
        notice_of_cross_appeal.last_filed = timezone.now()
    ...
    def post(self, request, notice_of_cross_appeal_id):
        ...
        notice_of_cross_appeal.transaction_id = transaction_id
        notice_of_cross_appeal.save()
        ...
        if redirect_url is not None:
            notice_of_cross_appeal.submission_id = submission_id 
            notice_of_cross_appeal.last_filed = timezone.now()            
            notice_of_cross_appeal.save()
