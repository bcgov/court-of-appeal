...
class Form8EFilingSubmitView(generics.GenericAPIView):
    ...
    def get_notice_of_application_to_vary_order_for_user(self, notice_of_application_to_vary_order_id, uid):
        try:
            notice_of_application_to_vary_order_query = NoticeOfApplicationToVaryOrder.objects.get(id=notice_of_application_to_vary_order_id, user_id=uid)
            return notice_of_application_to_vary_order_query
        except (NoticeOfApplicationToVaryOrder.DoesNotExist):
            logger.debug(no_record_found)
            return


    def _get_pdf_content(self, notice_of_application_to_vary_order, document_type):
        outgoing_documents = [] 
        ...
        try:                      
            prepared_pdf = FormPdf.objects.get(
                notice_of_application_to_vary_order_id=notice_of_application_to_vary_order.id, pdf_type=f"{current_document_type}"
            )
        ...
        document_json.update({"applicationId": notice_of_application_to_vary_order.id})
    ...
    def put(self, request, notice_of_application_to_vary_order_id):
        uid = request.user.id
        body = request.data
        ...
        if not notice_of_application_to_vary_order.submission_id or  not notice_of_application_to_vary_order.transaction_id:
            return HttpResponseNotFound("no record found")

        notice_of_application_to_vary_order.package_number = body.get("packageNumber")
        notice_of_application_to_vary_order.package_url = body.get("packageUrl")
        notice_of_application_to_vary_order.status="Submitted"
        notice_of_application_to_vary_order.last_filed = timezone.now()
    ...
    def post(self, request, notice_of_application_to_vary_order_id):
        ...
        document_type = "MBAV" # type Form8 for Efiling
        uid = request.user.id
        ...
        if notice_of_application_to_vary_order.package_number or notice_of_application_to_vary_order.package_url: 
            return JsonMessageResponse("This application has already been submitted.", status=500)

        outgoing_documents = self._get_pdf_content(notice_of_application_to_vary_order, document_type)               
        data_for_efiling = self.efiling_parsing.convert_data_for_efiling(
            request, notice_of_application_to_vary_order, outgoing_documents, document_type
        )
        
        # EFiling upload document.
        transaction_id = str(uuid.uuid4())
        notice_of_application_to_vary_order.transaction_id = transaction_id
        notice_of_application_to_vary_order.save()
        ...
        if redirect_url is not None:
            notice_of_application_to_vary_order.submission_id = submission_id 
            notice_of_application_to_vary_order.last_filed = timezone.now()            
            notice_of_application_to_vary_order.save()
