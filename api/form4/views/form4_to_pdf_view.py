...
class Form4ToPdfView(generics.GenericAPIView):
    ...
    def get_pdf_by_notice_of_application_id_and_type(self, id, pdf_type):
        try:
            pdf_result = FormPdf.objects.get(notice_of_application_id=id, pdf_type=pdf_type)
            return pdf_result
        except (FormPdf.DoesNotExist, NoticeOfApplication.DoesNotExist):
            LOGGER.debug(no_record_found)
            return
    


    def get(self, request, notice_of_application_id = None):
        ...
        notice_of_application_ids = request.query_params.getlist("id")                            

        if not notice_of_application_ids:        
            ...
            prepared_pdf = self.get_pdf_by_notice_of_application_id_and_type(notice_of_application_id, pdf_type)
            ...
            pdf_contents = list()
            for each_notice_of_application_id in notice_of_application_ids:
                notice_of_application = self.get_notice_of_application_for_user(each_notice_of_application_id, uid)
                if not notice_of_application:
                    continue

                prepared_pdf = self.get_pdf_by_notice_of_application_id_and_type(each_notice_of_application_id, pdf_type)
                if prepared_pdf is None:
                    continue

                pdf_contents.append({"id":each_notice_of_application_id, "type":pdf_type, "pdf": settings.ENCRYPTOR.decrypt(prepared_pdf.key_id, prepared_pdf.data)})
            if not pdf_contents:
                return HttpResponseNotFound(no_record_found)
    ...
    def post(self, request, notice_of_application_id):
        ...
        try:
            pdf_result = self.get_pdf_by_notice_of_application_id_and_type(notice_of_application_id, pdf_type)            
            ...
            if pdf_result:
                ...
                pdf_result = FormPdf(
                    notice_of_application=notice_of_application,
            ...
            notice_of_application.pdf_types = pdf_type
