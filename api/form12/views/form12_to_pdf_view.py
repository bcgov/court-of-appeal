...
class Form12ToPdfView(generics.GenericAPIView):
    ...
    def get_order_to_vary_order_of_justice_for_user(self, order_to_vary_order_of_justice_id, uid):
        try:
            ...
            return order_to_vary_order_of_justice
    ...
    def get_pdf_by_order_to_vary_order_of_justice_id_and_type(self, id, pdf_type):
        try:
            pdf_result = FormPdf.objects.get(order_to_vary_order_of_justice_id=id, pdf_type=pdf_type)
            return pdf_result
        except (FormPdf.DoesNotExist, OrderToVaryOrderOfJustice.DoesNotExist):
            LOGGER.debug(no_record_found)
            return
    


    def get(self, request, order_to_vary_order_of_justice_id = None):
        ...
        order_to_vary_order_of_justice_ids = request.query_params.getlist("id")                            

        if not order_to_vary_order_of_justice_ids:        
            ...
            prepared_pdf = self.get_pdf_by_order_to_vary_order_of_justice_id_and_type(order_to_vary_order_of_justice_id, pdf_type)
            ...
            pdf_contents = list()
            for each_order_to_vary_order_of_justice_id in order_to_vary_order_of_justice_ids:
                order_to_vary_order_of_justice = self.get_order_to_vary_order_of_justice_for_user(each_order_to_vary_order_of_justice_id, uid)
                if not order_to_vary_order_of_justice:
                    continue

                prepared_pdf = self.get_pdf_by_order_to_vary_order_of_justice_id_and_type(each_order_to_vary_order_of_justice_id, pdf_type)
                if prepared_pdf is None:
                    continue

                pdf_contents.append({"id":each_order_to_vary_order_of_justice_id, "type":pdf_type, "pdf": settings.ENCRYPTOR.decrypt(prepared_pdf.key_id, prepared_pdf.data)})
            if not pdf_contents:
                return HttpResponseNotFound(no_record_found)
    ...
    def post(self, request, order_to_vary_order_of_justice_id):
        ...
        try:
            pdf_result = self.get_pdf_by_order_to_vary_order_of_justice_id_and_type(order_to_vary_order_of_justice_id, pdf_type)            
            ...
            if pdf_result:
                ...
                pdf_result = FormPdf(
                    order_to_vary_order_of_justice=order_to_vary_order_of_justice,
            ...
            order_to_vary_order_of_justice.pdf_types = pdf_type
