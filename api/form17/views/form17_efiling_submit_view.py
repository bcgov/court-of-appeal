...
class Form17EFilingSubmitView(generics.GenericAPIView):
    ...
    def get_certificate_of_costs_for_user(self, certificate_of_costs_id, uid):
        try:
            certificate_of_costs_query = CertificateOfCosts.objects.get(id=certificate_of_costs_id, user_id=uid)
            return certificate_of_costs_query
        except (CertificateOfCosts.DoesNotExist):
            logger.debug(no_record_found)
            return


    def _get_pdf_content(self, certificate_of_costs, document_type):
        outgoing_documents = [] 
        ...
        try:                      
            prepared_pdf = FormPdf.objects.get(
                certificate_of_costs_id=certificate_of_costs.id, pdf_type=f"{current_document_type}"
            )
        ...
        document_json.update({"applicationId": certificate_of_costs.id})
    ...
    def put(self, request, certificate_of_costs_id):
        uid = request.user.id
        body = request.data
        ...
        if not certificate_of_costs.submission_id or  not certificate_of_costs.transaction_id:
            return HttpResponseNotFound("no record found")

        certificate_of_costs.package_number = body.get("packageNumber")
        certificate_of_costs.package_url = body.get("packageUrl")
        certificate_of_costs.status="Submitted"
        certificate_of_costs.last_filed = timezone.now()
    ...
    def post(self, request, certificate_of_costs_id):
        ...
        document_type = "COS" # type Form17 for Efiling
        uid = request.user.id
        ...
        if certificate_of_costs.package_number or certificate_of_costs.package_url: 
            return JsonMessageResponse("This application has already been submitted.", status=500)

        outgoing_documents = self._get_pdf_content(certificate_of_costs, document_type)               
        data_for_efiling = self.efiling_parsing.convert_data_for_efiling(
            request, certificate_of_costs, outgoing_documents, document_type
        )
        
        # EFiling upload document.
        transaction_id = str(uuid.uuid4())
        certificate_of_costs.transaction_id = transaction_id
        certificate_of_costs.save()
        ...
        if redirect_url is not None:
            certificate_of_costs.submission_id = submission_id 
            certificate_of_costs.last_filed = timezone.now()            
            certificate_of_costs.save()
