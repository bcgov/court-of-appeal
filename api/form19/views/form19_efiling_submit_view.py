...
class Form19EFilingSubmitView(generics.GenericAPIView):
    ...
    def get_withdraw_lawyer_for_user(self, withdraw_lawyer_id, uid):
        try:
            withdraw_lawyer_query = WithdrawLawyer.objects.get(id=withdraw_lawyer_id, user_id=uid)
            return withdraw_lawyer_query
        except (WithdrawLawyer.DoesNotExist):
            logger.debug(no_record_found)
            return


    def _get_pdf_content(self, withdraw_lawyer, document_type):
        outgoing_documents = [] 
        ...
        try:                      
            prepared_pdf = FormPdf.objects.get(
                withdraw_lawyer_id=withdraw_lawyer.id, pdf_type=f"{current_document_type}"
            )
        ...
        document_json.update({"applicationId": withdraw_lawyer.id})
    ...
    def put(self, request, withdraw_lawyer_id):
        uid = request.user.id
        body = request.data
        ...
        if not withdraw_lawyer.submission_id or  not withdraw_lawyer.transaction_id:
            return HttpResponseNotFound("no record found")

        withdraw_lawyer.package_number = body.get("packageNumber")
        withdraw_lawyer.package_url = body.get("packageUrl")
        withdraw_lawyer.status="Submitted"
        withdraw_lawyer.last_filed = timezone.now()
    ...
    def post(self, request, withdraw_lawyer_id):
        ...
        document_type = "CNWD" # type Form19 for Efiling
        uid = request.user.id
        ...
        if withdraw_lawyer.package_number or withdraw_lawyer.package_url: 
            return JsonMessageResponse("This application has already been submitted.", status=500)

        outgoing_documents = self._get_pdf_content(withdraw_lawyer, document_type)               
        data_for_efiling = self.efiling_parsing.convert_data_for_efiling(
            request, withdraw_lawyer, outgoing_documents, document_type
        )
        
        # EFiling upload document.
        transaction_id = str(uuid.uuid4())
        withdraw_lawyer.transaction_id = transaction_id
        withdraw_lawyer.save()
        ...
        if redirect_url is not None:
            withdraw_lawyer.submission_id = submission_id 
            withdraw_lawyer.last_filed = timezone.now()            
            withdraw_lawyer.save()
