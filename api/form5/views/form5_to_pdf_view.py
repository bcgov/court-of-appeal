...
from form5.models import NoticeOfHearing, FormPdf
...
class Form5ToPdfView(generics.GenericAPIView):
    ...
    def get_pdf_by_notice_id_and_type(self, id, pdf_type):
        try:
            pdf_result = FormPdf.objects.get(notice_of_hearing_id=id, pdf_type=pdf_type)
            return pdf_result
        except (FormPdf.DoesNotExist, NoticeOfHearing.DoesNotExist):
            LOGGER.debug(no_record_found)
            return
