...
class Form7ToPdfView(generics.GenericAPIView):
    ...
    def get(self, request, notice_of_urgent_application_id = None):
        ...
        if not notice_of_urgent_application_ids:        
            ...
            pdf_contents = list()
            for each_notice_of_urgent_application_id in notice_of_urgent_application_ids:
                notice_of_urgent_application = self.get_notice_of_urgent_application_for_user(each_notice_of_urgent_application_id, uid)
                if not notice_of_urgent_application:
                    continue

                prepared_pdf = self.get_pdf_by_notice_of_urgent_application_id_and_type(each_notice_of_urgent_application_id, pdf_type)
                if prepared_pdf is None:
                    continue

                pdf_contents.append({"id":each_notice_of_urgent_application_id, "type":pdf_type, "pdf": settings.ENCRYPTOR.decrypt(prepared_pdf.key_id, prepared_pdf.data)})
            if not pdf_contents:
                return HttpResponseNotFound(no_record_found)
