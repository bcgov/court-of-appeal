<template>
    <div> 
        ...
        <b-modal size="lg" v-model="confirmDelete" id="bv-modal-confirm-delete" header-class="bg-danger text-light">                    
            ...
            <h4 v-if="applicationsToDelete.length>0">Are you sure you want to delete the selected <b>"{{applicationsToDelete.join(', ')}}"</b> <b class="text-danger"> Notice of Change of Representation/Change of Address for Service </b> application<span v-if="applicationsToDelete.length>1" >s</span>?</h4>            
...
@Component
export default class TableForm18 extends Vue {
    ...
    public resumeApplication(fileInfo: documentInfoType) {
        ...
        this.UpdateCurrentNoticeOfRepChangeAddressId(caseId);  
    ...
    public downloadDocument(fileNumber?) {
        ...
        if(fileNumber || checkedFileIdsList.length>0){
            ...
            const pdf_type = 'FORM';
            const url = '/form18/form-print/'+filenum+'/?pdf_type='+pdf_type+pdfIds;
            const options = {
                responseType: "blob",
                headers: {
