<template>
    <div> 
        ...
        <b-row v-if="enableActions && documentsList.length" class="bg-form14 mb-2 py-1 mx-0">
            <b-col cols="10">
                <div 
        ...
        <b-row v-else-if="documentsList.length" class="bg-select mb-2 py-1 mx-0">
            <b-col cols="12">
                <div 
                    ...
                    class="p-0 text-center text-primary">Consent order to remove an appeal from the inactive list (Form 14) </div>
            </b-col>
        </b-row>
        ...
        <b-modal size="lg" v-model="confirmDelete" id="bv-modal-confirm-delete" header-class="bg-danger text-light">                    
            ...
            <h4 v-if="applicationsToDelete.length>0">Are you sure you want to delete the selected <b>"{{applicationsToDelete.join(', ')}}"</b> <b class="text-danger"> Consent order to remove an appeal from the inactive list </b> application<span v-if="applicationsToDelete.length>1" >s</span>?</h4>            
...
@Component
export default class TableForm14 extends Vue {
    ...
    public resumeApplication(fileInfo: documentInfoType) {
        ...
        this.UpdateCurrentConsentOrderRemoveInactiveId(caseId);             
    ...
    public downloadDocument(fileNumber?) {
        ...
        if(fileNumber || checkedFileIdsList.length>0){
            ...
            const pdf_type = 'FORM';
            const url = '/form14/form-print/'+filenum+'/?pdf_type='+pdf_type+pdfIds;
            const options = {
                responseType: "blob",
                headers: {
