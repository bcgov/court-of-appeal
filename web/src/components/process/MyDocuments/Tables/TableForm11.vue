<template>
    <div> 
        ...
        <b-row v-if="enableActions && documentsList.length" class="bg-form11 mb-2 py-1 mx-0">
            <b-col cols="10">
                <div 
...
@Component
export default class TableForm11 extends Vue {
    ...
    @form11State.Action
    public UpdateCurrentOrderMultipleJusticesId!: (newCurrentOrdeMultipleJusticesId: string) => void
    ...
    public resumeApplication(fileInfo: documentInfoType) {
        ...
        this.UpdateCurrentOrderMultipleJusticesId(caseId);             
    ...
    public downloadDocument(fileNumber?) {
        ...
        if(fileNumber || checkedFileIdsList.length>0){
            ...
            const pdf_type = 'FORM';
            const url = '/form11/form-print/'+filenum+'/?pdf_type='+pdf_type+pdfIds;
            const options = {
                responseType: "blob",
                headers: {
