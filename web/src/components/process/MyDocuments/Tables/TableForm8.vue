<template>
    <div> 
        ...
        <b-row v-else-if="documentsList.length" class="bg-select mb-0 py-1 mx-0">
            <b-col cols="12">
                <div style="font-weight:600; line-height:0.6rem; font-size:10pt; margin:0 0 0 0rem;" class="p-0 text-center text-primary">Notice of Application to Vary an Order of a Justice (Form 8) </div>
            </b-col>
        </b-row>
        ...
        <b-modal size="lg" v-model="confirmDelete" id="bv-modal-confirm-delete" header-class="bg-danger text-light">                    
            ...
            <h4 v-if="applicationsToDelete.length>0">Are you sure you want to delete the selected <b>"{{applicationsToDelete.join(', ')}}"</b> <b class="text-danger"> Notice of Application to Vary an Order of a Justice </b> application<span v-if="applicationsToDelete.length>1" >s</span>?</h4>            
...
@Component
export default class TableForm8 extends Vue {
    ...
    public resumeApplication(fileInfo: documentInfoType) {
        ...
        this.UpdateCurrentNoticeOfApplicationToVaryId(caseId);  
