<template>
    <div> 
        ...
        <b-row v-if="enableActions && documentsList.length" class="bg-form20 mb-2 py-1 mx-0">
            <b-col cols="10">
                <div 
        ...
        <b-modal size="lg" v-model="confirmDelete" id="bv-modal-confirm-delete" header-class="bg-danger text-light">                    
            ...
            <h4 v-if="applicationsToDelete.length>0">Are you sure you want to delete the selected <b>"{{applicationsToDelete.join(', ')}}"</b> <b class="text-danger"> Notice of Objection to Withdrawal </b> application<span v-if="applicationsToDelete.length>1" >s</span>?</h4>            
...
@Component
export default class TableForm20 extends Vue {
    ...
    public downloadDocument(fileNumber?) {
        ...
        if(fileNumber || checkedFileIdsList.length>0){
            ...
            const pdf_type = 'FORM';
            const url = '/form20/form-print/'+filenum+'/?pdf_type='+pdf_type+pdfIds;
            const options = {
                responseType: "blob",
                headers: {
