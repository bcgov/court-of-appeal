<template>
    <div> 
        ...
        <b-row v-if="enableActions && documentsList.length" class="bg-form9 mb-2 py-1 mx-0">
            <b-col cols="10">
                <div style="font-weight:600; font-size:14pt; margin:0 0 0 18rem;" class="p-0 text-center">Requisition (Form 9)</div>
            </b-col>
            <b-col  cols="2">
                <b-button 
        ...
        <b-row v-else-if="documentsList.length" class="bg-select mb-2 py-1 mx-0">
            <b-col cols="12">
                <div style="font-weight:600; line-height:1rem; font-size:12pt; margin:0 0 0 0rem;" class="p-0 text-center text-primary">Requisition (Form 9) </div>
            </b-col>
        </b-row>
...
@Component
export default class TableForm9 extends Vue {
    ...
    public resumeApplication(fileInfo: documentInfoType) {
        ...
        this.UpdateCurrentRequisitionId(caseId);  
    ...
    public downloadDocument(fileNumber?) {
        ...
        if(fileNumber || checkedFileIdsList.length>0){
            ...
            const pdf_type = 'FORM';
            const url = '/form9/form-print/'+filenum+'/?pdf_type='+pdf_type+pdfIds;
            const options = {
                responseType: "blob",
                headers: {
