<template>
    <b-card v-if="dataReady" header-tag="header" bg-variant="light" border-variant="white" style="width: 80rem;" class="mx-auto">
        ...
        <b-card-header header-bg-variant="light" header-border-variant="white">            
            <form-13-process-header v-bind:stepsCompleted="stepsCompleted"/>
        </b-card-header>
        ...
        <b-card border-variant="light" bg-variant="light" class="mt-3 mx-4">
            <form-13 v-bind:caseId="currentConsentOrderExtendTimeId" @navigateToSubmitPage="navigateToSubmitPage()" @pdfReady="pdfReady=true" />            
        </b-card> 
...
import Form13ProcessHeader from "@/components/process/Form13/components/Form13ProcessHeader.vue";
import Form13 from "@/components/process/Form13/components/pdf/Form13.vue"

import { form13StatusInfoType } from '@/types/Information/Form13';
...
export default class PreviewForm13 extends Vue {

    @form13State.State
    public currentConsentOrderExtendTimeId: string;

    stepsCompleted = {} as form13StatusInfoType;
    ...
    public navigateToSubmitPage() {
        this.$router.push({name: "proceed-form13" }); 
    }
}
</script>
