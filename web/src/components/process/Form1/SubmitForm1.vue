    ...
    <b-card v-if="mountedData" header-tag="header" bg-variant="light" border-variant="white" style="width: 80rem;" class="mx-auto">
        ...
        <b-card text-variant="dark" class="my-2 mx-5 bg-light border-light">
            ...
            <b-row class="ml-2 mb-1" style="font-size: 14px;">
                Do you have additional documents (Notice Of Application, affidavits) to submit?
                If so, proceed to upload.
            </b-row>
            <b-row class="ml-2 mb-1" style="font-size: 14px;">
                <b class="mr-2">Note: </b> Your Notice of Appeal will be uploaded for you and you do not need to add it here. 
            </b-row>
        ...
        <b-modal size="lg" v-model="showTypeOfDocuments" hide-header-close hide-header>
            <b-card style="border-radius:10px" class="bg-light">
                <h1 
                    ...
                    style="border-radius:10px; width:35rem; margin:0 auto; padding:1rem 0;">Specify the Type of Document(s)</h1>
                <div 
                    v-if="supportingFile" 
                    class="h3 my-4 text-center">
                    <div class="mb-3"> File Name(s): </div> 
                        <span 
                            v-for="(file,inx) in supportingFile" 
                            :key="inx" style="display:block;" 
                            class="my-2  p-0 h3 text-success"> {{file["name"]}} </span> 
                    </div>
...
export default class SubmitForm1 extends Vue {
    ...
    public submit() {
        ...
        const url = "/form1/efiling/"+this.currentNoticeOfAppealId+"/submit/";
        ...
        bodyFormData.append('document_type', GetForm1PdfType(this.form1Info));
