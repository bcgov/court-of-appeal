<template>
    <b-card v-if="dataReady" header-tag="header" bg-variant="light" border-variant="white" style="width: 80rem;" class="mx-auto">
        ...
        <b-card border-variant="light" bg-variant="light" class="mt-3 mx-4">
            <form-12 v-bind:caseId="currentOrderToVarySingleJusticeId" @navigateToSubmitPage="navigateToSubmitPage()" @pdfReady="pdfReady=true" />            
        </b-card> 
...
import Form12 from "@/components/process/Form12/components/pdf/Form12.vue"
...
export default class PreviewForm12 extends Vue {
    ...
    public navigateToSubmitPage() {
        this.$router.push({name: "proceed-form12" }); 
    }
}
</script>
