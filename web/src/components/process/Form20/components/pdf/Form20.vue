<template>
    <b-card v-if="dataReady" class="bg-light border-light" >  
        ...
        <b-card id="print" style="border:1px solid; border-radius:5px;" bg-variant="white" class="mt-4 mb-4 container" no-body>
            <form-20-layout v-bind:result="result"/>
        </b-card>
...
import Form20Layout from "./Form20Layout.vue";
...
export default class Form20 extends Vue {
    ...
    result = {} as form20DataInfoType;
    dataReady = false;
   
    mounted(){
        this.dataReady = false;
        this.result = this.form20Info;
        //TODO: remove above line and uncomment getForm20Data
        this.dataReady = true;
        // this.getForm20Data(); 
    }   
    ...
    public onPrint() { 
        const pdf_type = "NHA"
        const pdf_name = "form20-" + this.currentNoticeOfObjectionToWithdrawalId;
    ...
    public savePdf(){        
        const pdfType = "NHA"
        const pdfName ="FORM20"
        const url = '/form20/form-print/'+this.currentNoticeOfObjectionToWithdrawalId+'/?pdf_type='+pdfType
        const options = {
            responseType: "blob",
            headers: {
            "Content-Type": "application/json",
    ...
    public getForm20Data() {        
        ...
        .then((response) => {
            if(response?.data?.data){            
                ...
                this.UpdateForm20Info(this.result)                         
                this.dataReady = true;
